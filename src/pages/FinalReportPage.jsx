...
const roadAnalysisData = [
  {
    id: 1,
    start: "Juna Khedapati Chirghar, 1st Pulia",
    end: "Indra Colony, Rajasthan 34003",
    length: "0.37 KM",
    rating: 63.67,
    defects: 50,
    details: {
  ...
  {
    id: 2,
    start: "Old Jhanwar Rd, Chopasni Housing Board",
    end: "Bhati ki bhawri, Chopasni Housing Board",
    length: "1.29 KM",
    rating: 43.92,
    defects: 218,
    details: {
  ...
  return (
    <>
      <tr className="main-row" onClick={() => setIsExpanded(!isExpanded)}>
        ...
        <td>{row.length}</td>
        <td>
          <span className="rating-circle fair"></span>
          {row.rating}(Fair)
        </td>
        <td>
          {row.defects} {isExpanded ? <FiChevronUp /> : <FiChevronDown />}
        </td>
      </tr>
    ...
    <div className="report-page">
      <header className="report-header">
        ...
        <div className="date-display">
          <FiCalendar className="icon" /> Wed, 26 Jun 2024 12:36:45 GMT
        </div>
      </header>
      ...
      <main className="report-content">
        ...
        <section className="card-layout breakdown-section">
          <div className="breakdown-left">
            <h2>
              Total Defects Breakdown and Roads Status (According to IRC Rating)
            </h2>
          ...
          <div className="breakdown-right">
            {Object.entries(roadStatus).map(([status, value]) => (
              <div key={status} className="status-item">
                <span>
                  {status}: <strong>{value} KM</strong>
                </span>
                <div className="progress-bar-container">
                  <div
                    className={`progress-bar ${status.toLowerCase()}`}
                    style={{ width: `${(value / 7.75) * 100}%` }}
                  ></div>
                </div>
              </div>
            ))}
        ...
        <section className="card-layout">
          ...
          <table className="analysis-table">
            ...
            <tbody>
              {roadAnalysisData.map((row) => (
                <RoadAnalysisRow key={row.id} row={row} />
              ))}
            </tbody>
          </table>
        </section>
        ...
        <section className="charts-section">
          <div className="card-layout chart-card">
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie
                  ...
                  label
                >
                  {donutChartData1.map((entry, index) => (
            ...
            <p className="chart-label">
              12th Rd Cir, Sardarpura, Jodhpur... <strong>1 defects</strong>
            </p>
          </div>
          <div className="card-layout chart-card">
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie
                  ...
                  fill="#8884d8"
                >
                  {donutChartData2.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={entry.color} />
                  ))}
                </Pie>
                <Tooltip />
